...
func (repo *SQLiteStorageRootRepository) Save(ctx context.Context, item library.StorageRoot) error {
	row := storageRootRow{
		...
		Mode: string(item.Mode), Status: string(item.Status), LastCheckedAt: item.LastCheckedAt,
...
func toDomainStorageRoot(row storageRootRow) (library.StorageRoot, error) {
	return library.NewStorageRoot(library.StorageRootParams{
		ID: row.ID, CatalogID: row.CatalogID, Name: row.Name, Path: row.Path, VolumeID: row.VolumeID,
		Mode: row.Mode, Status: row.Status, LastCheckedAt: row.LastCheckedAt, LastError: row.LastError,
