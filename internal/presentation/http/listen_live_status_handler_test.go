...
func TestResolveListenLiveStatusFromHTML(t *testing.T) {
	tests := []struct {
		name   string
		html   string
		status string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := resolveListenLiveStatusFromHTML("TESTVID008H", tt.html)
...
func TestListenLiveStatusVideoIDs(t *testing.T) {
	query := make(url.Values)
	query["id"] = []string{"TESTVID008H", "TESTVID008H"}
	query["ids"] = []string{"TESTVID002B,TESTVID010J"}
	videoIDs, err := listenLiveStatusVideoIDs(query)
	if err != nil {
