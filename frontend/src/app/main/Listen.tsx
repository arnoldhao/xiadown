...
export function ListenPage(props: ListenPageProps) {
  ...
  const shouldAcceptMuseSnapshot = React.useCallback(
    (snapshot: ListenPlaybackSnapshot) => {
      ...
      return hasMuseSession || playbackModeRef.current === "muse";
    },
    [],
  );
  ...
  const { projection: musePlayback, applySnapshot: applyOnlinePlaybackSnapshot } =
  ...
  React.useEffect(() => {
    if (mode !== "muse" || artistBrowsePage || normalizedQuery.length < 2) {
      setSearchItems([]);
      setSearchArtists([]);
      setSearchPlaylists([]);
